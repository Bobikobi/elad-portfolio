...
export default function ServicesWorld({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="text-start">
      <p className="mb-5 text-sm text-[var(--color-star-white)]/70">{t('services.subtitle', locale)}</p>
      <div className="space-y-3">
        {SERVICES.map((s) => (
          <div key={s.key} className="rounded-2xl border border-white/8 p-4" style={{ background: 'rgba(238,241,255,0.02)' }}>
            ...
            <p className="mt-1.5 text-xs leading-relaxed text-[var(--color-star-white)]/65">{t(`services.${s.key}.desc`, locale)}</p>
            {s.detail && (
              <Link href={path(s.detail)} className="mt-2.5 inline-flex items-center gap-1 text-xs text-[var(--color-core-gold)]/80 transition-colors hover:text-[var(--color-core-gold)]">
