...
1. התקנת Vercel CLI (`npm i -g vercel`) — כרגע לא מותקן, ובלעדיו אין `vercel integration` / `vercel env pull`.
2. `vercel link` → `vercel integration add neon` (או מה-dashboard). מזריק `DATABASE_URL` אוטומטית ל-3 הסביבות.
...
### M7 (רשות) — התראה על ליד חדש

בלי זה תגלה לידים רק כשתיכנס לאזור האישי. הכי פשוט: webhook לטלגרם או מייל דרך Resend (Vercel Marketplace, קטגוריית `messaging`). לא חוסם שום שלב אחר.
...
- `GSC_SERVICE_ACCOUNT_JSON` **לא** כקובץ בריפו — ה-guard חוסם `service-account*.json` וגם `-----BEGIN PRIVATE KEY-----`. env בלבד.
...
- מצב נוכחי: הענף `feat/f3-f4`, יש `docs/briefs/f4-1-brief.md` לא-מעוקב. לסגור אותו לפני שמתחילים.

---

## 7. החלטות פתוחות

1. **Auth** — Basic Auth (זהה ליער עד, ~30 דקות) מול התחברות אמיתית (Clerk דרך Marketplace, ~2 שעות, נראה מקצועי אם אי-פעם תראה את המסך ללקוח). ההמלצה: Basic Auth עכשיו, זה משתמש אחד.
2. **תמלילי צ'אט** — לשמור הכל, או רק שיחות שהניבו ליד? ההמלצה: רק אלה שהניבו ליד. פחות חשיפת פרטיות, כמעט אותו ערך.
3. **M7 התראות** — טלגרם (מיידי, חינם) או מייל? ההמלצה: טלגרם.
4. **תרגום מדריכים** — לחכות לנתוני M5 לפני שמשקיעים בתרגום, או לתרגם 2 מובילים מיד?
