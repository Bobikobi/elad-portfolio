...
- Prominences: 5 one-rooted flame sprites -> 3 two-rooted loops (grow / hold / drain over 18-26 s, roots hidden by the disc).
...
## Measurement (p7-sun, 1440x900 DPR 2, real GPU, deployed preview of 75bed8c, run twice, identical to localhost)

| # | criterion | before (P7 signed) | now | verdict |
|---|---|---|---|---|
| S2 | core peak >= 215, blown < 1%, core/limb >= 1.5 | 224, 0%, 2.16x | 228, 0.0000%, 1.77x | PASS |
| S3 | corona >= 2x sky, corners <= 3.1% | 2.955x, 0.000% | 3.247x, 0.000% | PASS |
...
| S6 | tier law: calls, triangles, median | 69/64, 167,134/146,549, 16.700 ms | 68/63, 167,132/146,547, 16.700 ms | flagged only because the counts DROPPED by one call (3 loops + 1 corona instead of 5 flames); median unchanged |
